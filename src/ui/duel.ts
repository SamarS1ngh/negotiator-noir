import type { AngleId, Band, DuelState, MoodState, Opponent, OpponentType, Risk, Script } from '../domain/types';
import { riskOf } from '../domain/engine';
import { mountFace } from './face';
import type { Stage } from './face';

// The stage (his frames + the camera + the room) must SURVIVE re-renders. The
// old build wiped root.innerHTML on every beat and re-mounted it, which killed
// every animation the instant it started — that's why the scene never moved.
// We keep the world mounted and only rebuild the HUD on top of it.
const stages = new WeakMap<HTMLElement, { scene: HTMLElement; bg: HTMLElement; stage: Stage; oppId: string }>();
...
export interface DuelView {
  ...
  cutToHim?: CutToHim;
  // YOUR read of him, called before you sat down. The risk shown on each line
  // is derived from THIS — your belief — not from the truth. Read him wrong and
  // your own instruments lie to you all night. That's the cost of a bad call.
...
// plus the two clocks that can end you: your own nerve and his patience ----
function buildTop(opp: Opponent, state: DuelState, view: DuelView): HTMLElement {
  ...
  // the two ways HE beats YOU — thin, quiet, always there
  const mine = document.createElement('div');
  mine.className = 'minebars';
  mine.appendChild(miniBar('you', 'your nerve', view.yourNervePct));
  mine.appendChild(miniBar('pat', 'his patience', view.patiencePct));
  top.appendChild(mine);

  return top;
}

function miniBar(cls: string, label: string, pct: number): HTMLElement {
...
function buildReads(state: DuelState, opp: Opponent, reaction?: DuelReaction): HTMLElement {
  ...
  if (subtext) {
    ...
    const k = document.createElement('span');
    k.className = 'k';
    // NOT "what it means" — the game doesn't interpret him for you. This is
    // what you SAW. What it means is your problem.
    k.textContent = '⌕ what you notice';
    const t = document.createElement('span');
    t.className = 't';
...
function buildRecordBtn(state: DuelState, on: DuelHandlers): HTMLElement {
  ...
  // No cue about WHAT is in there. The record is your notebook, not an alarm —
  // it never lights up to tell you he just contradicted himself. You have to
  // catch that yourself, by remembering what he said.
...
function buildModal(
  ...
  for (const l of script.lines.filter((line) => line.angleId === angle)) {
    // risk is computed against YOUR READ of him, not the truth — a bad call
    // means these dots are confidently wrong
    const risk = riskOf(state, { ...opp, type: believedType }, l);
...
export function renderDuel(
  ...
  let cached = stages.get(root);

  // mount the world once; after that we only swap the HUD on top of it
  if (!cached || cached.oppId !== opp.id || !root.contains(cached.scene)) {
    root.innerHTML = '';
    root.className = 'duel-screen';
    const scene = document.createElement('div');
    scene.className = 'scene';
    const bg = document.createElement('div');
    bg.className = 'bg';
    const stage = mountFace(bg, opp);
    scene.appendChild(bg);
    root.appendChild(scene);
    cached = { scene, bg, stage, oppId: opp.id };
    stages.set(root, cached);
  } else {
    root.className = 'duel-screen';
    // tear down only the HUD — the camera + his frames keep running
    for (const child of [...cached.scene.children]) {
      if (child !== cached.bg) child.remove();
    }
    for (const child of [...root.children]) {
      if (child !== cached.scene) child.remove();
    }
  }

  const scene = cached.scene;
  const face = cached.stage;
  face.setMood(state.mood);

  scene.appendChild(buildTop(opp, state, view));
  ...
  if (view.selectedAngle) {
    ...
    root.appendChild(buildModal(state, opp, script, view.selectedAngle, on, view.believedType));
  }
